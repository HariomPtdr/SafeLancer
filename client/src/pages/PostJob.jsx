...
const SectionHeader = ({ num, title, subtitle }) => (
  <div className="flex items-center gap-3 mb-5">
    <div className="w-7 h-7 rounded-lg text-white text-xs font-bold flex items-center justify-center flex-shrink-0" style={{ background: 'linear-gradient(135deg, #FF6803, #AE3A02)' }}>{num}</div>
    <div>
      <h2 className="text-sm font-semibold text-white">{title}</h2>
      {subtitle && <p className="text-xs mt-0.5" style={{ color: '#BFBFBF' }}>{subtitle}</p>}
    </div>
  </div>
)
    ...
    <div className="min-h-screen" style={{ background: 'transparent' }}>
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        <div className="mb-5">
          ...
          <p className="text-sm mt-1" style={{ color: '#BFBFBF' }}>Define your project phases upfront — freelancers see the full scope before applying</p>
        </div>
          ...
          <div className="rounded-xl p-4 mb-5" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold" style={{ color: '#FF6803' }}>Complete your profile first</p>
                ...
                <div className="w-full rounded-full h-1.5 mt-2 overflow-hidden" style={{ background: 'rgba(245,158,11,0.15)' }}>
                  <div className="h-1.5 rounded-full transition-all" style={{ width: `${completion}%`, background: '#FF6803' }} />
                </div>
              </div>
              <Link to="/profile/setup" className="flex-shrink-0 text-xs font-medium px-3 py-1.5 rounded-lg transition-colors" style={{ background: '#FF6803', color: '#000' }}>
                Complete Profile
              </Link>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className={isBlocked ? 'opacity-50 pointer-events-none select-none space-y-5' : 'space-y-5'}>
          ...
          <div className="dark-card p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Job Title</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Category</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Experience Level</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Project Description</label>
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <span className="text-xs" style={{ color: '#6b5445' }}>{form.description.length} chars</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Total Budget (₹)</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Overall Deadline</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5" style={{ color: '#BFBFBF' }}>Required Skills</label>
          ...
          <div className="dark-card p-6">
            ...
            {budget > 0 && (
              <div className="p-3 rounded-lg text-sm" style={{ background: '#120a02', border: '1px solid rgba(255,104,3,0.06)' }}>
                <div className="flex justify-between" style={{ color: '#BFBFBF' }}>
          ...
          <div className="dark-card p-6">
            ...
            {budget > 0 && (
              <div className="mb-5 p-4 rounded-lg" style={{ background: '#120a02', border: '1px solid rgba(255,104,3,0.06)' }}>
                <p className="text-xs font-semibold mb-2 text-white">Payment Breakdown</p>
                <div className="space-y-1">
                  <div className="flex justify-between text-xs" style={{ color: '#BFBFBF' }}>
                    <span className="font-medium">Advance ({form.advancePercent}%)</span>
                    <span className="font-semibold text-white">₹{advanceAmount.toLocaleString()}</span>
                  </div>
                  {phases.map((p, i) => (
                    <div key={i} className="flex justify-between text-xs" style={{ color: '#6b5445' }}>
                  ...
                  <div className={`flex justify-between text-xs font-semibold mt-2 pt-2 ${Math.abs(totalPercent - 100) > 0.5 ? 'text-red-400' : 'text-white'}`} style={{ borderTop: '1px solid rgba(255,104,3,0.06)' }}>
            ...
            <div className="space-y-4">
              {phases.map((phase, i) => {
                ...
                return (
                <div key={i} className="rounded-xl p-4" style={{ border: '1px solid rgba(255,104,3,0.06)' }}>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-white">Phase {i + 1}</span>
                      {isLast && <span className="text-xs px-2 py-0.5 rounded-md" style={{ background: '#120a02', color: '#6b5445' }}>Final phase</span>}
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-medium mb-1 block" style={{ color: '#BFBFBF' }}>Phase Title</label>
                      ...
                      <div>
                        <label className="text-xs font-medium mb-1 block" style={{ color: '#BFBFBF' }}>Deliverable Type</label>
                    ...
                    <div>
                      <label className="text-xs font-medium mb-1 block" style={{ color: '#BFBFBF' }}>Phase Guideline / Acceptance Criteria</label>
                      ...
                      {phase.guidelineHash && (
                        <div className="flex items-center gap-1.5 mt-1.5 px-2 py-1 rounded-md" style={{ background: 'rgba(255,104,3,0.08)', border: '1px solid rgba(255,104,3,0.12)' }}>
                          <svg className="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: '#BFBFBF' }}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        <label className="text-xs font-medium mb-1 block" style={{ color: '#BFBFBF' }}>Budget %</label>
                        ...
                        {isLast && <p className="text-xs mt-0.5" style={{ color: '#6b5445' }}>Auto: remaining {lastPhasePercent}%</p>}
                        {!isLast && errors[`phase_${i}_budget`] && <p className="text-red-400 text-xs mt-0.5">{errors[`phase_${i}_budget`]}</p>}
                      </div>
                      <div>
                        <label className="text-xs font-medium mb-1 block" style={{ color: '#BFBFBF' }}>Phase Deadline</label>
                        ...
                        {isLast && <p className="text-xs mt-0.5" style={{ color: '#6b5445' }}>Auto: matches project deadline</p>}
                        {!isLast && errors[`phase_${i}_deadline`] && <p className="text-red-400 text-xs mt-0.5">{errors[`phase_${i}_deadline`]}</p>}
                      </div>
                      <div>
                        <label className="text-xs font-medium mb-1 block" style={{ color: '#BFBFBF' }}>Max Revisions</label>
            ...
            <button type="button" onClick={addPhase}
              ...
              style={{ border: '2px dashed rgba(255,104,3,0.10)', color: '#6b5445', background: 'transparent' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,104,3,0.25)'; e.currentTarget.style.color = '#BFBFBF' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,104,3,0.10)'; e.currentTarget.style.color = '#1c1008' }}>
              + Add Phase
            </button>
          </div>
          ...
          <div className="dark-card p-6">
            ...
            {referenceFiles.length > 0 && (
              <div className="space-y-2 mb-4">
                {referenceFiles.map((f, i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-lg" style={{ background: '#120a02', border: '1px solid rgba(255,104,3,0.06)' }}>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white truncate">{f.originalName}</p>
                      <p className="text-xs font-mono mt-0.5 truncate" style={{ color: '#BFBFBF' }}>{f.fileHash}</p>
                    </div>
                    <button type="button" onClick={() => setReferenceFiles(referenceFiles.filter((_, idx) => idx !== i))}
                      ...
                      onMouseLeave={e => e.currentTarget.style.color = '#1c1008'}>×</button>
                  </div>
                ))}
              </div>
            )}
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploadingFile || referenceFiles.length >= 5}
              ...
              style={{ border: '2px dashed rgba(255,104,3,0.10)', color: '#6b5445', background: 'transparent' }}
              onMouseEnter={e => { if (!uploadingFile && referenceFiles.length < 5) { e.currentTarget.style.borderColor = 'rgba(255,104,3,0.25)'; e.currentTarget.style.color = '#BFBFBF' } }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,104,3,0.10)'; e.currentTarget.style.color = '#1c1008' }}>
          ...
          <div className="dark-card p-6">
            ...
            {form.title && budget > 0 && (
              <div className="mb-5 p-4 rounded-lg space-y-2" style={{ background: '#120a02', border: '1px solid rgba(255,104,3,0.06)' }}>
                {[
                  ['Title', form.title],
            ...
            <div className="p-3 rounded-lg mb-5" style={{ background: 'rgba(255,104,3,0.08)', border: '1px solid rgba(255,104,3,0.14)' }}>
              <p className="text-xs" style={{ color: '#BFBFBF' }}>Scope Lock</p>
              <p className="text-xs mt-1" style={{ color: '#BFBFBF' }}>After posting, a SHA-256 hash of your job title, description, and all phase guidelines will be generated and stored. This creates a tamper-proof record that protects both parties in any dispute.</p>
            </div>
